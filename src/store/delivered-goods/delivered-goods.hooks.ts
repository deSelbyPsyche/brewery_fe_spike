...
import { DeliveryId } from "../batches/batches.types";
import { getDeliveredGoodsByDeliveryId } from "./delivered-goods.selectors";

export const useFetchDeliveredGoods = (delivery_id: DeliveryId) =>
  useApiFetch({
    requestFn: async () => {
      const res: AxiosResponse<DeliveredGoodsFetchResponseData> = await axios.get(
...
export const useAddDeliveredGood = () =>
  useApiMutate({
    requestFn: async (data: DeliveredGoodsAddParams) => {
      const res: AxiosResponse<DeliveredGoodsFetchResponseData> = await axios.post(
        `${process.env.REACT_APP_API_URL}/delivered-goods`,
        data
      );
